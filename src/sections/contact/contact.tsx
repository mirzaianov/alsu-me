...
import { DottedMap } from '../../shared/ui/dotted-map/dotted-map';
import {
  contactContent,
  type ContactMapMarker,
} from '../../content/ru/contact';
import styles from './contact.module.css';

type ContactActionId = (typeof contactContent.actions)[number]['id'];

const renderContactActionIcon = (id: ContactActionId) => {
  if (id === 'telegram') {
    return (
      <svg
        viewBox="0 0 24 24"
...
const Contact = () => {
  return (
    ...
    >
      <h2 className={clsx('title', styles.title)}>{contactContent.title}</h2>
      <article className={styles.content}>
        <div className={styles.mapBlock}>
          <p className={styles.mapText}>{contactContent.mapText}</p>
          <div className={styles.mapFrame}>
            <DottedMap<ContactMapMarker>
              aria-label={contactContent.mapAriaLabel}
              className={styles.map}
              dotColor="var(--text-90)"
              dotRadius={0.16}
              markerColor="var(--primary-30)"
              markers={contactContent.mapMarkers}
              pulse
          ...
          <div className={styles.actions}>
            {contactContent.actions.map((action) => (
              <Button
                ...
                key={action.id}
              >
                {renderContactActionIcon(action.id)}
