...
- The hero keeps viewport-filling minimum height on mobile/tablet, while desktop layouts from `min-width: 1061px` opt out of hero min-height and flex growth in favor of explicit padding and bounded gaps. At `1920px` and wider, root spacing and font tokens follow a consistent 1.125x extra-large scale rounded to the project pixel rhythm, while hero-only slider slots, note size, and portrait size are adjusted to match the scaled text. [Reason why added: KAN-131 stopped desktop hero content from stretching vertically while keeping extra-large typography and gaps systematic instead of one-off.]
