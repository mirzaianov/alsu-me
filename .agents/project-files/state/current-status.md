...
- The hero keeps viewport-filling minimum height on mobile/tablet, while desktop layouts from `min-width: 1061px` opt out of hero min-height and flex growth in favor of explicit padding and bounded gaps. At `1921px` and wider, root spacing and font tokens follow a consistent extra-large scale while the global section max-width layer remains capped by `--container-wide` through `--section-max-extra-wide`; hero-only slider slots, note size, portrait size, larger heading-to-content gaps, full-width About row with a badge-aware photo lane, large button sizing, Contact CTA equal-width action stack, equal-height content-sized Services card tracks, content-sized Pricing cards, fixed testimonial card heights, extra-wide pricing-card width, and tutor-pricing action row alignment are adjusted to match the scaled text, while Services/Pricing `h3` subtitles are not capped to copy-wrapper widths. [Reason why added: KAN-131 stopped desktop hero content from stretching vertically while keeping extra-large typography, gaps, containers, buttons, and fixed-format UI elements systematic instead of one-off.]
