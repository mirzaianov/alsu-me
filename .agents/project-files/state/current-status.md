...
- Raster UI images render through `next/image` with `quality={100}` allowed by `next.config.ts`; the hero/about portraits use mobile assets as the default source and tablet assets from `min-width: 577px` through `getImageProps`/`picture` art direction. SVG logo/icon assets are rendered deliberately through `next/image` with per-image SVG optimization disabled, and the shared brand logo does not request image preload. [Reason why added: default Next image quality and losing the mobile-first portrait source selection reduced visual fidelity after the KAN-110 migration.]
