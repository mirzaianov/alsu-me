...
- The hero logo marquee is a client component that renders one accessible set of logos and uses a GSAP timeline to wrap individual logo items horizontally with width-aware `xPercent` transforms. [Reason why added: duplicated CSS tracks could show a shift or blink at the loop reset seam on Android Chrome and iPadOS Safari.]
