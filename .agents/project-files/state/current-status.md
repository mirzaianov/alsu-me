...
- The testimonials carousel uses GSAP `matchMedia()` to derive loop duration from responsive pixel-per-second targets: 20px/s on mobile, 28px/s on tablet, and 44px/s on desktop. [Reason why added: a fixed pixel speed feels faster on small screens because it crosses a larger share of the viewport per second, while desktop needs a higher raw speed to avoid feeling stalled.]
