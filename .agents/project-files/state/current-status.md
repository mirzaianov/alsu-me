...
- The hero keeps viewport-filling minimum height on mobile/tablet, while desktop layouts from `min-width: 1061px` opt out of hero min-height and flex growth in favor of explicit padding and bounded gaps. At `1920px` and wider, the hero, header, and footer switch together to `--container-extra-wide` while ordinary desktop shell widths remain unchanged. [Reason why added: KAN-131 stopped desktop hero content from stretching vertically and adds extra-wide framing without changing the usual desktop layout.]
